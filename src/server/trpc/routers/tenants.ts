...
export const tenantsRouter = router({
  ...
  get: adminProcedure.input(z.object({ id: z.string().uuid() })).query(async ({ input }) => {
    const { rows } = await adminPool.query<Tenant>("SELECT * FROM tenants WHERE id = $1", [
      input.id,
    ]);
    if (rows.length === 0) {
  ...
  create: adminProcedure.input(CreateTenantSchema).mutation(async ({ input }) => {
    const { rows: existing } = await adminPool.query("SELECT id FROM tenants WHERE slug = $1", [
      input.slug,
    ]);
  ...
  stats: adminProcedure.input(z.object({ id: z.string().uuid() })).query(async ({ input }) => {
    const { rows: tenantRows } = await adminPool.query("SELECT id FROM tenants WHERE id = $1", [
      input.id,
    ]);
    ...
    const { rows: tagStats } = await adminPool.query(
      "SELECT COUNT(*) as count FROM tags WHERE tenant_id = $1",
      [input.id]
    );
    const { rows: keyStats } = await adminPool.query(
  ...
  createApiKey: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const { rows } = await adminPool.query("SELECT id FROM tenants WHERE id = $1", [
        input.tenantId,
      ]);
      if (rows.length === 0) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Tenant not found" });
